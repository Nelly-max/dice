    ...
    public function index()
{
    ...
    */
    ...
    |--------------------------------------------------------------------------
    | IMPORTANT:
    | If a shop has already been selected by the cart,
    | only show products from that shop.
    |--------------------------------------------------------------------------
    */
    ...
    */
    ...
    ->map(function ($product) {
        ...
        */
        ...
        if ($product->size_value && $product->quantity_unit) {
            $variant =
                rtrim(rtrim($product->size_value, '0'), '.') .
                $product->quantity_unit;

        } elseif ($product->weight_value && $product->weight_unit) {
            $variant =
                rtrim(rtrim($product->weight_value, '0'), '.') .
                $product->weight_unit;
        }
        ...
        if (
            ...
            $startValid =
                ...
                $now->greaterThanOrEqualTo(
                    Carbon::parse($product->discount_start)
                );

            $stopValid =
                is_null($product->discount_stop) ||
                $now->lessThanOrEqualTo(
                    Carbon::parse($product->discount_stop)
                );
            ...
            if ($startValid && $stopValid) {
                ...
                $finalPrice = max(
                    0,
                    $originalPrice - (float) $product->discount
                );
    ...
    return view('HomeMarket.home', compact(
        'products',
        'selectedBusinessAccount'
    ));
}
