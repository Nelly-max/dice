...
use App\Models\Customer\OrderCustomerDetail;
...
class PaymentProcessingService
{
     ...
     */
    private function createOrder(CheckoutInvoice $invoice): Order
    {
        $order = DB::connection('customer')->transaction(function () use ($invoice) {
            ...
            */
            ...
            OrderCustomerDetail::updateOrCreate(
                ...
                [
                    ...
                    'alternative_phone_number' => $invoice->alternative_phone_number,
            ...
            */
            ...
            /*
            |--------------------------------------------------------------------------
            | ✨ Cart Cleanup Logic Injected Here
            |--------------------------------------------------------------------------
            */
            $cartItemIds = $checkoutItems->pluck('cart_item_id')->filter()->toArray();

            if (!empty($cartItemIds)) {
                // Using DB query builder to avoid model mass-assignment overhead during batch delete
